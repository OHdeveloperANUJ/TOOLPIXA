...
export default function PersonalLoanEmi() {
  ...
  useEffect(() => {
    fetch('/api/stats?toolId=' + 'personal-loan-emi')
      .then(r => r.json())
  ...
  const handleSave = () => {
    ...
    try {
      ...
      const newItem = {
        ...
        toolId: 'personal-loan-emi-calculator',
  ...
  const handleDownloadCsv = () => {
    ...
    a.href = url;
    a.download = `personal_loan_${scheduleView}_schedule_${currency.code}.csv`;
    a.click();
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-white/5 backdrop-blur-2xl p-6 lg:p-8 rounded-[32px] border border-white/10 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input type="range" min="10000" max="5000000" step="10000" value={principal} onChange={(e) => setPrincipalStr(e.target.value)} className="w-full accent-cyan-400 h-1 cursor-pointer bg-white/10 rounded-full appearance-none" />
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <button 
                onClick={() => handleQuickFill(100000)}
                className="px-3 py-1 bg-white/5 hover:bg-cyan-500/20 border border-white/10 hover:border-cyan-500/30 rounded-full text-xs text-cyan-300 font-bold transition-all flex items-center gap-0.5"
              >
                <Plus size={10} /> 1 Lakh
              </button>
              <button 
                onClick={() => handleQuickFill(200000)}
                className="px-3 py-1 bg-white/5 hover:bg-cyan-500/20 border border-white/10 hover:border-cyan-500/30 rounded-full text-xs text-cyan-300 font-bold transition-all flex items-center gap-0.5"
              >
                <Plus size={10} /> 2 Lakhs
              </button>
              <button 
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              />
              <input type="range" min="1" max="7" step="1" value={tenureYears} onChange={(e) => setTenureYearsStr(e.target.value)} className="w-full accent-cyan-400 h-1 cursor-pointer bg-white/10 rounded-full appearance-none" />
            </div>
          </div>
